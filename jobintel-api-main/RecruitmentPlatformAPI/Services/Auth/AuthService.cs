...
namespace RecruitmentPlatformAPI.Services.Auth
{
    public class AuthService : IAuthService
    {
            ...
            try
            {
                ...
                return new AuthResponseDto
                {
                    ...
                    User = new UserInfoDto
                    {
                        ...
                        AuthProvider = newUser.AuthProvider.ToString()
                    }
                };
            }
            catch (Exception ex)
        ...
        public async Task<AuthResponseDto> UpdateNameAsync(int userId, UpdateNameDto updateNameDto)
        {
            try
            {
                ...
                var trimmedFirstName = updateNameDto.FirstName.Trim();
                var trimmedLastName = updateNameDto.LastName.Trim();
                ...
                _logger.LogInformation($"Name updated for user: {user.Email}");
            ...
            {
                _logger.LogError(ex, "Name update failed due to database error");
                return new AuthResponseDto { Success = false, Message = "Failed to update name. Please try again later." };
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Name update failed due to unexpected error");
                return new AuthResponseDto { Success = false, Message = "Failed to update name. Please try again later." };
            }
        }
        ...
        public async Task<AuthResponseDto> ChangePasswordAsync(int userId, ChangePasswordDto changePasswordDto)
        {
            ...
            {
                _logger.LogError(ex, "Password change failed due to database error");
                return new AuthResponseDto { Success = false, Message = "Failed to change password. Please try again later." };
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Password change failed due to unexpected error");
                return new AuthResponseDto { Success = false, Message = "Failed to change password. Please try again later." };
            }
        }

        public async Task<AuthResponseDto> DeactivateAccountAsync(int userId, string authProvider, DeactivateAccountDto dto)
        {
            try
            {
                var user = await _context.Users.FindAsync(userId);
                ...
                _logger.LogInformation($"Account deactivated for user: {user.Email}");
            ...
            {
                _logger.LogError(ex, "Account deactivation failed due to database error");
                return new AuthResponseDto { Success = false, Message = "Failed to deactivate account. Please try again later." };
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Account deactivation failed due to unexpected error");
                return new AuthResponseDto { Success = false, Message = "Failed to deactivate account. Please try again later." };
            }
        }

        public async Task<AuthResponseDto> DeleteAccountAsync(int userId, string authProvider, DeleteAccountDto dto)
        {
            using var transaction = await _context.Database.BeginTransactionAsync();
            try
            {
                ...
                // Delete role-specific data first (cascade will handle most, but be explicit)
                if (accountType == AccountType.JobSeeker)
                {
                    ...
                    if (jobSeeker != null)
                    {
                        ...
                        var experiences = await _context.Experiences.Where(e => e.JobSeekerId == jobSeeker.Id).ToListAsync();
                        _context.Experiences.RemoveRange(experiences);

                        var educations = await _context.Educations.Where(e => e.JobSeekerId == jobSeeker.Id).ToListAsync();
                        _context.Educations.RemoveRange(educations);

                        var projects = await _context.Projects.Where(p => p.JobSeekerId == jobSeeker.Id).ToListAsync();
                        _context.Projects.RemoveRange(projects);

                        var certificates = await _context.Certificates.Where(c => c.JobSeekerId == jobSeeker.Id).ToListAsync();
                        _context.Certificates.RemoveRange(certificates);

                        var skills = await _context.JobSeekerSkills.Where(s => s.JobSeekerId == jobSeeker.Id).ToListAsync();
                        _context.JobSeekerSkills.RemoveRange(skills);

                        var socialAccounts = await _context.SocialAccounts.Where(s => s.JobSeekerId == jobSeeker.Id).ToListAsync();
                        _context.SocialAccounts.RemoveRange(socialAccounts);

                        var resumes = await _context.Resumes.Where(r => r.JobSeekerId == jobSeeker.Id).ToListAsync();
                        _context.Resumes.RemoveRange(resumes);

                        // Clean up entities with Restrict FK behaviour so the
                        // User delete doesn't fail with a FK constraint violation.
                        var shortlistedAsCandidate = await _context.ShortlistedCandidates
                            .Where(s => s.JobSeekerId == jobSeeker.Id).ToListAsync();
                        _context.ShortlistedCandidates.RemoveRange(shortlistedAsCandidate);

                        var recommendationsAsCandidate = await _context.Recommendations
                            .Where(r => r.JobSeekerId == jobSeeker.Id).ToListAsync();
                        _context.Recommendations.RemoveRange(recommendationsAsCandidate);
                ...
                else
                {
                    ...
                    if (recruiter != null)
                    {
                        ...
                        var jobs = await _context.Jobs.Where(j => j.RecruiterId == recruiter.Id).ToListAsync();
                        foreach (var job in jobs)
                        {
                            var jobSkills = await _context.JobSkills.Where(s => s.JobId == job.Id).ToListAsync();
                            _context.JobSkills.RemoveRange(jobSkills);

                            var recommendations = await _context.Recommendations.Where(r => r.JobId == job.Id).ToListAsync();
                            _context.Recommendations.RemoveRange(recommendations);
                        }
                        _context.Jobs.RemoveRange(jobs);

                        var shortlisted = await _context.ShortlistedCandidates.Where(s => s.RecruiterId == recruiter.Id).ToListAsync();
                        _context.ShortlistedCandidates.RemoveRange(shortlisted);
                ...
                if (accountType == AccountType.JobSeeker)
                {
                    var jobSeekerForViews = await _context.JobSeekers.FirstOrDefaultAsync(j => j.UserId == userId);
                    if (jobSeekerForViews != null)
                    {
                        var profileViews = await _context.ProfileViews.Where(p => p.JobSeekerId == jobSeekerForViews.Id).ToListAsync();
                        _context.ProfileViews.RemoveRange(profileViews);
                    }
                }
                else
                {
                    var recruiterForViews = await _context.Recruiters.FirstOrDefaultAsync(r => r.UserId == userId);
                    if (recruiterForViews != null)
                    {
                        var profileViews = await _context.ProfileViews.Where(p => p.ViewerRecruiterId == recruiterForViews.Id).ToListAsync();
                        _context.ProfileViews.RemoveRange(profileViews);
                    }
                }

                // Delete auth-related data
                var verifications = await _context.EmailVerifications.Where(e => e.UserId == userId).ToListAsync();
                _context.EmailVerifications.RemoveRange(verifications);

                var passwordResets = await _context.PasswordResets.Where(p => p.UserId == userId).ToListAsync();
                ...
                _logger.LogInformation($"Account permanently deleted for user: {userEmail}");

                return new AuthResponseDto
                {
                    Success = true,
                    Message = "Account deleted permanently. All associated data has been removed."
                };
            }
            catch (DbUpdateException ex)
            {
                await transaction.RollbackAsync();
                _logger.LogError(ex, "Account deletion failed due to database error");
                return new AuthResponseDto { Success = false, Message = "Failed to delete account. Please try again later." };
            }
            catch (Exception ex)
            {
                await transaction.RollbackAsync();
                _logger.LogError(ex, "Account deletion failed due to unexpected error");
                return new AuthResponseDto { Success = false, Message = "Failed to delete account. Please try again later." };
            }
        }
    }
